...
| 2026-09-10 | Update MariaDB guidance on the Requirements page: raise the supported minimum from the EOL 10.3 to 11.4 LTS (11.8 LTS recommended), add an LTS support-status table and EOL warning, add `DB_CONNECTION=mysql` vs `mariadb` guidance, add database-creation SQL and a MySQL-to-MariaDB dump migration tip; add a database-connection tip to the CLI installer prompts on the Installation page | `docs/2.2/introduction/requirements.md`, `docs/2.2/introduction/installation.md` | Done |
